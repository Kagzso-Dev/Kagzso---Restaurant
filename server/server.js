...
io.on('connection', (socket) => {
    ...
    // Join branch-specific room for tenant isolation (with authorization)
    socket.on('join-branch', (branchId) => {
        if (!branchId) return;
        authorizedRoomJoin(socket, branchId);
    });

    // Join role-specific room for targeted notifications (with authorization)
    socket.on('join-role', ({ branchId, role }) => {
        if (!branchId || !role) return;
        authorizedRoleJoin(socket, branchId, role);
    });
